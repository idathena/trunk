...
static bool mmo_auth_fromsql(AccountDB_SQL *db, struct mmo_account *acc, int account_id)
{
	...
	Sql_GetData(sql_handle, 10, &data, NULL); safestrncpy(acc->lastlogin, (data ? data : ""), sizeof(acc->lastlogin));
	Sql_GetData(sql_handle, 11, &data, NULL); safestrncpy(acc->last_ip, data, sizeof(acc->last_ip));
	Sql_GetData(sql_handle, 12, &data, NULL); safestrncpy(acc->birthdate, (data ? data : ""), sizeof(acc->birthdate));
