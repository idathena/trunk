...
void unit_run_hit(struct block_list *bl, struct status_change *sc, struct map_session_data *sd, enum sc_type type) {
	...
	if (type == SC_RUN)
		clif_status_change(bl, SI_TING, 1, 0, 0, 0, 0);
	...
	if (type == SC_RUN) {
		...
		clif_status_change(bl, SI_TING, 0, 0, 0, 0, 0);
	} else if (sd) {
		clif_fixpos(bl);
...
int unit_movepos(struct block_list *bl, short dst_x, short dst_y, int easy, bool checkpath)
{
	...
	if( sd ) {
		...
		if( sd->status.pet_id > 0 && sd->pd && sd->pd->pet.intimate >= PETINTIMATE_AWKWARD ) { //Check if pet needs to be teleported [Skotlex]
			int flag = 0;
...
int unit_blown_immune(struct block_list *bl, int flag)
{
	...
	switch (bl->type) {
		case BL_MOB:
			if ((flag&0x1) && status_bl_has_mode(bl, MD_KNOCKBACK_IMMUNE) && ((flag&0x2) || !(battle_config.skill_trap_type&0x2)))
				return 2; //Immune can't be knocked back
			break;
		case BL_PC: {
...
int unit_set_walkdelay(struct block_list *bl, unsigned int tick, int delay, int type)
{
	...
	if (type) {
		if (bl->type == BL_MOB && status_bl_has_mode(bl, MD_STATUS_IMMUNE))
...
int unit_set_target(struct unit_data *ud, int target_id)
{
	...
	if(ud->target != target_id) {
		...
		if(ud->target && (target = map_id2bl(ud->target)) && (ux = unit_bl2ud(target)) && ux->target_count > 0)
			ux->target_count--;
		if(target_id && (target = map_id2bl(target_id)) && (ux = unit_bl2ud(target)) && ux->target_count < UCHAR_MAX)
			ux->target_count++;
	}
...
int unit_remove_map_(struct block_list *bl, clr_type clrtype, const char *file, int line, const char *func)
{
	...
	switch (bl->type) {
		...
		case BL_PET: {
				...
				if (pd->pet.intimate < PETINTIMATE_AWKWARD && !(pd->master && !pd->master->state.active)) {
