...
static int script_getitem_randomoption(struct script_state *st, struct item *it, const char *funcname, int x) {
	...
	for( i = 0; i < opt_id_n && i < MAX_ITEM_RDM_OPT; i++ ) {
		it->option[i].id = (short)__64BPRTSIZE(get_val2(st, reference_uid(opt_id_id, opt_id_idx + i), reference_getref(opt_id)));
		script_removetop(st,-1,0);
		it->option[i].value = (short)__64BPRTSIZE(get_val2(st, reference_uid(opt_val_id, opt_val_idx + i), reference_getref(opt_val)));
		script_removetop(st,-1,0);
		it->option[i].param = (char)__64BPRTSIZE(get_val2(st, reference_uid(opt_param_id, opt_param_idx + i), reference_getref(opt_param)));
		script_removetop(st,-1,0);
	}
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(sc_end)
{
	...
	if( type >= 0 && type < SC_MAX ) {
		...
		switch( type ) {
			...
			case SC_ATTHASTE_CASH:		case SC_ALL_RIDING_REUSE_LIMIT:	case SC_REUSE_LIMIT_A:
			case SC_REUSE_LIMIT_B:		case SC_REUSE_LIMIT_C:		case SC_REUSE_LIMIT_D:
			case SC_REUSE_LIMIT_E:		case SC_REUSE_LIMIT_F:		case SC_REUSE_LIMIT_G:
			case SC_REUSE_LIMIT_H:		case SC_REUSE_MILLENNIUMSHIELD:	case SC_REUSE_CRUSHSTRIKE:
			case SC_REUSE_REFRESH:		case SC_REUSE_STORMBLAST:	case SC_REUSE_LIMIT_MTF:
			case SC_REUSE_LIMIT_ECL:	case SC_REUSE_LIMIT_RECALL:	case SC_REUSE_LIMIT_ASPD_POTION:
	...
	switch( bl->type ) {
		case BL_MOB:
			...
			switch( type ) {
				...
				case 2: status_set_hp(bl, (unsigned int)value, 0); if( battle_config.show_mob_info&3 ) clif_name_area(&md->bl); break;
				case 3: status_set_maxhp(bl, (unsigned int)value, 0); if( battle_config.show_mob_info&3 ) clif_name_area(&md->bl); break;
