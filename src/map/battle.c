...
int battle_delay_damage(unsigned int tick, int amotion, struct block_list *src, struct block_list *target, int attack_type, uint16 skill_id, uint16 skill_lv, int64 damage, enum damage_lv dmg_lv, int ddelay, bool additional_effects, bool isvanishdamage, bool isspdamage)
{
	...
	if( ((d_tbl && check_distance_bl(target, d_tbl, sc->data[SC_DEVOTION]->val3)) ||
		...
		skill_id != CR_REFLECTSHIELD && skill_id != PA_PRESSURE &&
...
int64 battle_calc_damage_sub(struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	if( battle_skill_check_bypass_modifiers(src,bl,d,skill_id) || skill_id == SJ_NOVAEXPLOSING )
		return damage;
	...
	if( sc && sc->count ) {
		...
		if( sc->data[SC_FOGWALL] && skill_id &&
			!(skill_get_inf(skill_id)&INF_GROUND_SKILL) && !(skill_get_nk(skill_id)&NK_SPLASH) )
		...
		if( sc->data[SC_AETERNA] && skill_id != PF_SOULBURN ) {
			...
			if( src->type != BL_MER || !skill_id ) //Only doubles basic attack damage if source is BL_MER
				damage *= 2;
		}
		...
		if( flag&BF_WEAPON && (sce = sc->data[SC_TUNAPARTY]) ) {
			sce->val2 -= (int)cap_value(damage,INT_MIN,INT_MAX);
			if( sce->val2 <= 0 ) {
				damage = -sce->val2;
...
int64 battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	if( sc && sc->count ) {
		...
		if( (sce = sc->data[SC_LIGHTNINGWALK]) && (flag&(BF_LONG|BF_MAGIC)) == BF_LONG && rnd()%100 < sce->val2 ) {
			...
			if( !map_flag_gvg2(src->m) && !mapdata[src->m].flag.battleground && unit_movepos(bl,src->x - dx[dir],src->y - dy[dir],1,true) ) {
				clif_blown(bl,src);
				unit_setdir(bl,dir);
			}
	...
	if( damage > 0 ) {
		if( battle_config.pk_mode && bl->type == BL_PC && src->type == BL_PC && mapdata[bl->m].flag.pvp ) { //PK damage rates
...
{
	if(mapdata[m].skill_count) {
		int i;

		skill_id = skill_dummy2skill_id(skill_id);
		ARR_FIND(0, mapdata[m].skill_count, i, mapdata[m].skills[i]->skill_id == skill_id);
		if(i < mapdata[m].skill_count)
			return mapdata[m].skills[i]->modifier;
	}
	return 0;
}
...
static int battle_skill_damage_skill(struct block_list *src, struct block_list *target, uint16 skill_id)
{
	...
	struct map_data *mapd = &mapdata[m];
...
static int battle_skill_damage_map(struct block_list *src, struct block_list *target, uint16 skill_id)
{
	int rate = 0;
	uint8 i = 0;
	struct map_data *mapd = &mapdata[src->m];
	...
	switch(skill_id) {
		...
		case LG_HESPERUSLIT:
			if(sc && sc->data[SC_BANDING] && ((battle_config.hesperuslit_bonus_stack && sc->data[SC_BANDING]->val2 >= 5) || sc->data[SC_BANDING]->val2 == 5))
				element = ELE_HOLY;
			break;
		case RL_H_MINE: //Force deals fire damage if activated by RL_FLICKER
		...
		switch(skill_id) {
			...
			default:
				if(!skill_id && (battle_config.attack_attr_none&src->type))
					return; //Non-pc (mob, pet, homun) basic attacks are non-elemental, they deal 100% against all defense elements
#ifdef RENEWAL
	...
	switch(skill_id) {
		...
		case LK_JOINTBEAT:
			...
			if ((wd->miscflag&BREAK_NECK) || (tsc && tsc->data[SC_JOINTBEAT] && tsc->data[SC_JOINTBEAT]->val2&BREAK_NECK))
				skillration *= 2; //The 2x damage is only for the BREAK_NECK ailment
			break;
 ...
 */
static void battle_attack_cannonball(struct Damage *wd, struct block_list *src, uint16 skill_id)
{
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	short index = -1;
 ...
 * @return weapon data
 */
static void battle_calc_damage_modifiers(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	if( !wd->damage2 ) {
...
static void battle_calc_attack_plant(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	battle_calc_damage_modifiers(wd, src, target, skill_id, skill_lv);
}
...
struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
	...
	if(!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true))
		...
		battle_attack_cannonball(&wd, src, skill_id);
			...
			default:
				...
				switch(skill_id) {
					...
					case LG_SHIELDSPELL: //[(Caster's Base Level x 4) + (Shield MDEF x 100) + (Caster's INT x 2)] %
						skillratio += -100 + 4 * status_get_lv(src) + 100 * (sd ? sd->bonus.shieldmdef : 1) + 2 * sstatus->int_;
						break;
					...
					case SO_VARETYR_SPEAR:
						...
						skillratio += -100 + sstatus->int_ * skill_lv + 50 * (sd ? pc_checkskill(sd, SA_LIGHTNINGLOADER) : 5);
						RE_LVL_DMOD(100);
						if(sc && sc->data[SC_BLAST_OPTION])
...
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, unsigned int tick, int flag) {
	...
	if (sc && sc->data[SC__AUTOSHADOWSPELL] && rnd()%100 < sc->data[SC__AUTOSHADOWSPELL]->val4) {
		...
		uint16 skill_lv = sc->data[SC__AUTOSHADOWSPELL]->val3;
		int type;

		if ((type = skill_get_casttype(skill_id)) == CAST_GROUND &&
			!skill_pos_maxcount_check(src,target->x,target->y,skill_id,skill_lv,BL_PC,true) ) {
			map_freeblock_unlock();
			return wd.dmg_lv;
		}
	...
	if (damage > 0) {
		...
		if (sc && sc->count) {
			...
			if ((sce = sc->data[SC_DUPLELIGHT]) && rnd()%100 < 10 + 2 * sce->val1 &&
				wd.flag&BF_SHORT) { //Activates it only from melee damage
				uint16 skill_id;
				...
				skill_attack(skill_get_type(skill_id),src,src,target,skill_id,sce->val1,tick,flag);
			}
			if ((sce = sc->data[SC_FALLINGSTAR]) && rnd()%100 < sce->val2)
				skill_castend_damage_id(src,src,SJ_FALLINGSTAR_ATK,sce->val1,tick,flag);
		}
		if (tsc) {
			struct status_change_entry *sce = NULL;

			if ((sce = tsc->data[SC_POISONREACT]) &&
				(rnd()%100 < sce->val3 || sstatus->def_ele == ELE_POISON) &&
...
int battle_check_target(struct block_list *src, struct block_list *target, int flag)
{
				...
				if( (skill_get_inf2(su->group->skill_id)&INF2_TRAP) && su->group->unit_id != UNT_USED_TRAPS ) {
					if( !skill_id ) {
						...
						switch( skill_id ) {
							...
							case NC_AXETORNADO:
							case SR_SKYNETBLOW:
								if( !mapdata[m].flag.pvp && !mapdata[m].flag.gvg )
		...
		case BL_PC: {
				...
				if( t_bl->id != s_bl->id ) {
					if( sd->state.killer ) {
						...
						strip_enemy = 0;
					} else if( sd->duel_group && !((!battle_config.duel_allow_pvp && mapdata[m].flag.pvp) ||
	...
	if( map_flag_vs(m) ) { //Check rivalry settings
		...
		if( mapdata[m].flag.battleground ) {
		...
		if( (flag&(BCT_PARTY|BCT_ENEMY)) ) {
			...
			if( s_party && s_party == status_get_party_id(t_bl) &&
				!(mapdata[m].flag.pvp && mapdata[m].flag.pvp_noparty) &&
				!(map_flag_gvg(m) && mapdata[m].flag.gvg_noparty) &&
				(!mapdata[m].flag.battleground || sbg_id == tbg_id) )
				state |= BCT_PARTY;
			else
				state |= BCT_ENEMY;
		}
		if( (flag&(BCT_GUILD|BCT_ENEMY)) ) {
			...
			if( !(mapdata[m].flag.pvp && mapdata[m].flag.pvp_noguild) &&
		...
		if( state&BCT_PARTY ) {
			...
			if( (mapdata[m].flag.gvg_castle || mapdata[m].flag.gvg_te_castle) &&
				(!s_guild || !t_guild || (s_guild != t_guild && ((flag&BCT_SAMEGUILD) || !guild_isallied(s_guild, t_guild)))) ) {
				state |= BCT_ENEMY;
				strip_enemy = 0;
			}
		...
		if( state&BCT_ENEMY ) {
			if( mapdata[m].flag.battleground && sbg_id && sbg_id == tbg_id )
				state &= ~BCT_ENEMY;
