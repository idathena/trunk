...
static int storage_reconnect_sub(DBKey key, DBData *data, va_list ap)
{
	...
	if (stor->dirty && !stor->status) //Save closed storages
 ...
 * @param sd : player
 * @param stor : Storage data
 * @param index : idx on storage to remove the item from
...
{
	if (!stor->u.items_storage[index].nameid || stor->u.items_storage[index].amount < amount)
		return 1;

	stor->u.items_storage[index].amount -= amount;
	stor->dirty = true;

	if (!stor->u.items_storage[index].amount) {
 ...
 * @param sd : player
 * @return check e_guild_storage_flags
 */
char storage_guild_storageopen(struct map_session_data *sd)
{
	...
	if (!(gstor = guild2storage2(sd->status.guild_id))
#ifdef OFFICIAL_GUILD_STORAGE
...
bool storage_guild_additem2(struct s_storage *stor, struct item *item_data, int amount)
{
	...
	if (!item_data->nameid || amount <= 0 || !(id = itemdb_exists(item_data->nameid)))
		return false;
	...
	if (itemdb_isstackable2(id)) { //Stackable
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_guild[i], item_data)) { //Set the amount, make it fit with max amount
				int da = (id->stack.guildstorage ? id->stack.amount : MAX_AMOUNT) - stor->u.items_guild[i].amount;

				amount = min(amount, da);
				if (amount != item_data->amount)
					ShowWarning("storage_guild_additem2: Stack limit reached! Altered amount of item \""CL_WHITE"%s"CL_RESET"\" (%d). '"CL_WHITE"%d"CL_RESET"' -> '"CL_WHITE"%d"CL_RESET"'.\n", id->name, id->nameid, item_data->amount, amount);
				stor->u.items_guild[i].amount += amount;
				stor->dirty = true;
				return true;
			}
 ...
 * @param sd : player
 * @param stor : guild_storage
 * @param index : index of item in guild storage
...
bool storage_guild_delitem(struct map_session_data *sd, struct s_storage *stor, int index, int amount)
{
	...
	if (!stor->u.items_guild[index].amount) {
		memset(&stor->u.items_guild[index], 0, sizeof(stor->u.items_guild[0]));
		stor->amount--;
		clif_updatestorageamount(sd, stor->amount, stor->max_amount);
	}
...
void storage_guild_storageget(struct map_session_data *sd, int index, int amount)
{
	...
	if (!stor->u.items_guild[index].nameid)
		return;
	...
	if ((flag = pc_additem(sd, &stor->u.items_guild[index], amount, LOG_TYPE_GSTORAGE)) == ADDITEM_SUCCESS)
...
void storage_guild_storageaddfromcart(struct map_session_data *sd, int index, int amount)
{
	...
	if (!sd->cart.u.items_cart[index].nameid)
		return;
...
void storage_guild_storagesaved(int guild_id)
{
	...
	if ((stor = guild2storage2(guild_id)) && stor->dirty && !stor->status) //Storage has been correctly saved
		stor->dirty = false;
}
