...
int guild_invite(struct map_session_data *sd, struct map_session_data *tsd) {
	...
	if( tsd->status.guild_id > 0 || tsd->guild_invite > 0 ||
		(map_flag_gvg(tsd->bl.m) && !battle_config.guild_castle_invite) ) {
...
int guild_leave(struct map_session_data *sd, int guild_id, int account_id, int char_id, const char *mes)
{
	...
	if( sd->status.account_id != account_id || sd->status.char_id != char_id || sd->status.guild_id != guild_id ||
		(map_flag_gvg(sd->bl.m) && !battle_config.guild_castle_expulsion) )
		return 0;
...
int guild_expulsion(struct map_session_data *sd, int guild_id, int account_id, int char_id, const char *mes)
{
	...
	if( tsd && tsd->status.char_id == char_id && map_flag_gvg(tsd->bl.m) && !battle_config.guild_castle_expulsion )
		return 0;
...
void guild_retrieveitembound(uint32 char_id, uint32 account_id, int guild_id)
{
	...
	if( sd ) { //Character is online
		...
		if( j ) {
			...
			if( stor && stor->status ) { //Close the storage first if someone open it
				for( i = 0; i < g->max_member; i++ ) {
			...
			for( i = 0; i < j; i++ ) { //Loop the matching items, storage_guild_additem takes care of opening storage
				if( stor )
...
int guild_gm_change(int guild_id, uint32 char_id)
{
	...
	if (i == MAX_GUILD) //Not part of the guild
		return 0;
...
int guild_gm_changed(int guild_id, int account_id, int char_id, time_t time)
{
	...
	if (g->member[0].sd && g->member[0].sd->fd) {
		clif_displaymessage(g->member[0].sd->fd, msg_txt(g->member[0].sd, 679)); // "You have become the Guild Master!"
