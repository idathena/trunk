...
int channel_delete(struct Channel *channel, bool force) {
	...
	switch (channel->type) {
		case CHAN_TYPE_MAP:
			mapdata[channel->m].channel = NULL;
			aFree(channel);
			break;
		case CHAN_TYPE_ALLY: {
...
int channel_join(struct Channel *channel, struct map_session_data *sd) {
	...
	if (channel->type == CHAN_TYPE_PRIVATE && db_size(channel->users) >= channel_config.private_channel.max_member) {
		...
		sprintf(output, msg_txt(753), channel->name, channel_config.private_channel.max_member); // You cannot join channel '%s'. Limit of %d has been met.
		clif_displaymessage(sd->fd, output);
		return -4;
	}
	...
	if (sd->stealth)
		...
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(754), channel->alias, sd->status.name); // %s %s has joined.
...
int channel_mjoin(struct map_session_data *sd) {
	...
	if (!mapdata[sd->bl.m].channel)
		mapdata[sd->bl.m].channel = channel_create_simple(NULL, NULL, CHAN_TYPE_MAP, sd->bl.m);

	if (mapdata[sd->bl.m].channel->opt&CHAN_OPT_ANNOUNCE_SELF) {
		sprintf(mout, msg_txt(1435), mapdata[sd->bl.m].channel->name, mapdata[sd->bl.m].name); // You're now in the '#%s' channel for '%s'.
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], mout, false, SELF);
	}

	return channel_join(mapdata[sd->bl.m].channel, sd);
}
...
int channel_ajoin(struct guild *g) {
	...
	for (i = 0; i < MAX_GUILDALLIANCE; i++) {
		...
		if (ga->guild_id && !ga->opposition && (ag = guild_search(ga->guild_id))) {
...
int channel_gjoin(struct map_session_data *sd, int flag) {
	...
	if (flag&2) {
		...
		for (i = 0; i < MAX_GUILDALLIANCE; i++) {
			...
			if (ga->guild_id && !ga->opposition && (ag = guild_search(ga->guild_id)) ) //Only join allies
...
int channel_pcquit(struct map_session_data *sd, int type) {
	...
	if (type&4 && channel_config.map_tmpl.name[0] && channel_haspc(mapdata[sd->bl.m].channel, sd) == 1) //Quit map chan
		channel_clean(mapdata[sd->bl.m].channel, sd, 0);
...
int channel_send(struct Channel *channel, struct map_session_data *sd, const char *msg) {
	...
	if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) && channel->msg_delay && DIFF_TICK(sd->channel_tick[idx] + channel->msg_delay, gettick()) > 0) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(1455), false, SELF); // You're talking too fast!
		return -2;
	} else {
		char output[CHAT_SIZE_MAX];
...
int channel_chk(char *chname, char *chpass, int type) {
	if (type&1) { //Check name
		...
		if (strlen(chname) < 3 || strlen(chname) > CHAN_NAME_LENGTH)
			return -2;
		if ((type&2) &&
			(!strcmpi(chname + 1, channel_config.map_tmpl.name) ||
			!strcmpi(chname + 1, channel_config.ally_tmpl.name) ||
...
struct Channel *channel_name2channel(char *chname, struct map_session_data *sd, int flag) {
	if (channel_chk(chname, NULL, 1))
		return NULL;
	if (sd && !strcmpi(chname + 1, channel_config.map_tmpl.name)) {
		if (flag&1 && !mapdata[sd->bl.m].channel)
			mapdata[sd->bl.m].channel = channel_create_simple(NULL, NULL, CHAN_TYPE_MAP, sd->bl.m);
		if (flag&2 && channel_pc_haschan(sd, mapdata[sd->bl.m].channel) < 1)
			channel_mjoin(sd);
		return mapdata[sd->bl.m].channel;
	} else if (sd && !strcmpi(chname + 1, channel_config.ally_tmpl.name) && sd->guild) {
...
int channel_display_list(struct map_session_data *sd, char *options) {
	...
	if (options[0] != '\0' && !strcmpi(options, "colors")) {
		char msg[40];
		unsigned char k;
	...
	} else if (options[0] != '\0' && !strcmpi(options, "mine")) { //Display chan I'm into
		...
		if (channel_config.map_tmpl.name[0] && mapdata[sd->bl.m].channel) {
			sprintf(output, msg_txt(1409), mapdata[sd->bl.m].channel->name, db_size(mapdata[sd->bl.m].channel->users)); // - #%s (%d users)
			clif_displaymessage(sd->fd, output);
		}
		if (channel_config.ally_tmpl.name[0] && sd->status.guild_id) {
			...
			if (g && g->channel) {
				sprintf(output, msg_txt(1409), g->channel->name, db_size(((struct Channel *)g->channel)->users)); // - #%s (%d users)
				clif_displaymessage(sd->fd, output);
			}
		}
...
int channel_pcleave(struct map_session_data *sd, char *chname) {
	...
	if (!(channel->opt&CHAN_OPT_CAN_LEAVE)) {
		sprintf(output, msg_txt(755), chname); // You cannot leave channel '%s'.
		clif_displaymessage(sd->fd, output);
		return -1;
	}

	if (!channel_config.closing && (channel->opt&CHAN_OPT_ANNOUNCE_LEAVE)) {
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(756), channel->alias, sd->status.name); // %s %s left.
		clif_channel_msg(channel, output, channel->color);
	}
	switch (channel->type) {
...
int channel_pcjoin(struct map_session_data *sd, char *chname, char *pass) {
	...
	if (channel) {
		...
		if (channel_haspc(channel, sd) == 1) {
			...
			if (strcmp(channel->pass, pass)) { //Wrong pass entry
...
int channel_pccolor(struct map_session_data *sd, char *chname, char *color) {
	...
	if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
		if (channel->char_id != sd->status.char_id) {
			...
			sprintf(output, msg_txt(757), chname); // You cannot change the color for channel '%s'.
			clif_displaymessage(sd->fd, output);
			return -1;
		}
	}
...
int channel_pcban(struct map_session_data *sd, char *chname, char *pname, int flag) {
	...
	if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
		if (channel->char_id != sd->status.char_id) {
			...
			sprintf(output, msg_txt(758), chname); // You're not allowed to ban a player.
			clif_displaymessage(sd->fd, output);
			return -1;
		}
	}
...
int channel_pckick(struct map_session_data *sd, char *chname, char *pname) {
	...
	if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
		if (channel->char_id != sd->status.char_id) {
			...
			sprintf(output, msg_txt(759), chname); // You cannot kick a player from channel '%s'.
			clif_displaymessage(sd->fd, output);
		}
		return -1;
	}
	...
	if (channel->char_id == sd->status.char_id) {
		clif_displaymessage(sd->fd, msg_txt(760)); // You're not allowed to kick a player.
		return -1;
	}

	if (!channel_config.closing && (channel->opt&CHAN_OPT_ANNOUNCE_LEAVE)) {
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(761), channel->alias, tsd->status.name); // %s %s has been kicked.
		clif_channel_msg(channel, output, channel->color);
	}
...
int channel_pcsetopt(struct map_session_data *sd, char *chname, const char *option, const char *val) {
	...
	if (channel->char_id != sd->status.char_id && !pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
		sprintf(output, msg_txt(1412), chname); // You're not the owner of channel '%s'.
		clif_displaymessage(sd->fd, output);
		return -1;
	}
...
unsigned long channel_getColor(const char *color_str) {
	...
	for (i = 0; i < channel_config.colors_count; i++) {
		if (!strcmpi(channel_config.colors_name[i], color_str))
...
void channel_read_config(void) {
	...
	if (chan_setting) {
		...
		if (channels && (count = config_setting_length(channels)) > 0) {
			int i;
