 ...
 */
struct s_refine_materials {
	struct s_refine_cost cost;
	uint8 chance;
	struct s_refine_bs_blessing bs_blessing;
};
...
static bool clif_session_isValid(struct map_session_data *sd) {
	return (sd && session_isActive(sd->fd));
}
...
int clif_send(const uint8 *buf, int len, struct block_list *bl, enum send_target type) {
	...
	switch (type) {
		case ALL_CLIENT: //All player clients
			...
			while ((tsd = (TBL_PC *)mapit_next(iter))) {
...
static int clif_set_unit_idle(struct block_list *bl, unsigned char *buffer, bool spawn) {
	...
	if( battle_config.monster_hp_bars_info && !mapdata[bl->m].flag.hidemobhpbar && bl->type == BL_MOB && status_get_hp(bl) < status_get_max_hp(bl) ) {
...
#if PACKETVER >= 20091103
#if PACKETVER >= 20120221
	safestrncpy((char *)WBUFP(buf,64),name,NAME_LENGTH);
#else
	safestrncpy((char *)WBUFP(buf,55),name,NAME_LENGTH);
#endif
...
#if PACKETVER >= 20091103
#if PACKETVER >= 20120221
	safestrncpy((char *)WBUFP(buf,71),name,NAME_LENGTH);
#else
	safestrncpy((char *)WBUFP(buf,62),name,NAME_LENGTH);
#endif
	return WBUFW(buffer,2);
#else
	return packet_len(WBUFW(buffer,0));
...
void clif_class_change_target(struct block_list *bl, int class_, int type, enum send_target target, struct map_session_data *sd) {
	...
	if(!pcdb_checkid(class_)) { //Player classes yield missing sprites
		...
		WBUFB(buf,6) = type;
		WBUFL(buf,7) = class_;
		clif_send(buf,packet_len(0x1b0),(!sd ? bl : &sd->bl),target);
	}
}
...
static void clif_weather_check(struct map_session_data *sd)
{
	...
	if (mapdata[m].flag.snow ||
		mapdata[m].flag.clouds ||
		mapdata[m].flag.fog ||
		mapdata[m].flag.fireworks ||
		mapdata[m].flag.sakura ||
		mapdata[m].flag.leaves ||
		mapdata[m].flag.clouds2)
	{
		if (mapdata[m].flag.snow)
			clif_specialeffect_single(&sd->bl, EF_SNOW, fd);
		if (mapdata[m].flag.clouds)
			clif_specialeffect_single(&sd->bl, EF_CLOUD3, fd);
		if (mapdata[m].flag.clouds2)
			clif_specialeffect_single(&sd->bl, EF_CLOUD5, fd);
		if (mapdata[m].flag.fog)
			clif_specialeffect_single(&sd->bl, EF_CLOUD4, fd);
		if (mapdata[m].flag.fireworks) {
		...
		if (mapdata[m].flag.sakura)
			clif_specialeffect_single(&sd->bl, EF_SAKURA, fd);
		if (mapdata[m].flag.leaves)
...
int clif_spawn(struct block_list *bl)
{
	...
	switch (bl->type) {
		case BL_PC: {
				...
				if (sd->bg_id && mapdata[bl->m].flag.battleground)
...
void clif_hominfo(struct map_session_data *sd, struct homun_data *hd, int flag)
{
	...
	WBUFW(buf,0) = cmd;
	safestrncpy((char *)WBUFP(buf,2),hd->homunculus.name,NAME_LENGTH);
...
void clif_homskillinfo(struct map_session_data *sd, uint16 skill_id, int inf)
{
	...
	WFIFOW(fd,12) = skill_get_range2(&hd->bl, id, hd->homunculus.hskill[idx].lv, false);
...
void clif_quitsave(int fd,struct map_session_data *sd) {
	if (!battle_config.prevent_logout || !sd->canlog_tick || DIFF_TICK(gettick(), sd->canlog_tick) > battle_config.prevent_logout)
		map_quit(sd);
	else if (sd->fd) {
...
void clif_parse_NPCShopClosed(int fd, struct map_session_data *sd) {
	sd->npc_shopid = 0; //@TODO: State tracking?
}
...
void clif_scriptmes(struct map_session_data *sd, int npcid, const char *mes)
{
	...
	WFIFOW(fd,2) = slen;
	WFIFOL(fd,4) = npcid;
	safestrncpy((char *)WFIFOP(fd,8), mes, slen - 8);
	WFIFOSET(fd,WFIFOW(fd,2));
}
...
void clif_updatestatus(struct map_session_data *sd,int type)
{
	...
	switch( type ) { //Additional update packets that should be sent right after
		case SP_BASELEVEL:
			...
			break;
		case SP_HP:
			if( mapdata[sd->bl.m].hpmeter_visible )
				clif_hpmeter(sd);
...
void clif_getareachar_unit(struct map_session_data *sd,struct block_list *bl)
{
	...
	switch (bl->type) {
		case BL_PC: {
				...
				if (tsd->bg_id && mapdata[bl->m].flag.battleground)
		...
		case BL_MOB: {
				...
				if (battle_config.monster_hp_bars_info && !mapdata[bl->m].flag.hidemobhpbar) {
					int i;
...
void clif_skillinfo(struct map_session_data *sd, uint16 skill_id, int inf)
{
	...
	WFIFOW(fd,12) = skill_get_range2(&sd->bl, skill_id, sd->status.skill[skill_id].lv, false);
...
void clif_status_change(struct block_list *bl, int type, int flag, int tick, int val1, int val2, int val3) {
	...
	if (type == SI_ILLUSION && !battle_config.display_hallucination)
		return; //Disable Hallucination
...
void clif_broadcast(struct block_list *bl, const char *mes, int len, int type, enum send_target target)
{
	...
	safestrncpy((char *)WBUFP(buf,4 + lp),mes,len);
...
void clif_broadcast2(struct block_list *bl, const char *mes, int len, unsigned long fontColor, short fontType, short fontSize, short fontAlign, short fontY, enum send_target target)
{
	...
	WBUFW(buf,14) = fontY;
	safestrncpy((char *)WBUFP(buf,16), mes, len);
	clif_send(buf, WBUFW(buf,2), bl, target);
...
#if PACKETVER >= 20121010
	WBUFL(buf,4) = ((mapdata[bl->m].flag.pvp ? 1 : 0)<<0)| //PARTY - Show attack cursor on non-party members (PvP)
		(((mapdata[bl->m].flag.battleground || map_flag_gvg2(bl->m)) ? 1 : 0)<<1)| //GUILD - Show attack cursor on non-guild members (GvG)
		(((mapdata[bl->m].flag.battleground || map_flag_gvg2(bl->m)) ? 1 : 0)<<2)| //SIEGE - Show emblem over characters heads
		(((mapdata[bl->m].flag.nomineeffect || !map_flag_gvg2(bl->m)) ? 0 : 1)<<3)| //USE_SIMPLE_EFFECT - Automatically enable /mineffect
		((mapdata[bl->m].flag.nolockon ? 0 : (map_flag_vs(bl->m) ? ((mapdata[bl->m].flag.battleground || map_flag_gvg2(bl->m)) ? 0 : 1) : 0))<<4)| //DISABLE_LOCKON - Only allow attacks on other players with shift key or /ns active
		((mapdata[bl->m].flag.pvp ? 1 : 0)<<5)| //COUNT_PK - Show the PvP counter
		((mapdata[bl->m].flag.partylock ? 1 : 0)<<6)| //NO_PARTY_FORMATION - Prevents party creation/modification (Might be used for instance dungeons)
		((mapdata[bl->m].flag.battleground ? 1 : 0)<<7)| //BATTLEFIELD - Unknown (Does something for battlegrounds areas)
		((mapdata[bl->m].flag.nocostume ? 1 : 0)<<8)| //DISABLE_COSTUMEITEM - Disable costume sprites
		((mapdata[bl->m].flag.nousecart ? 0 : 1)<<9)| //USECART - Allow opening cart inventory (Well force it to always allow it)
		((mapdata[bl->m].flag.nosumstarmiracle ? 0 : 1)<<10); //SUNMOONSTAR_MIRACLE - Unknown - (Guessing it blocks Star Gladiator's Miracle from activating)
...
void clif_parse_BankOpen(int fd, struct map_session_data *sd) {
	...
	if(!battle_config.feature_banking) {
		clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1510),false,SELF); // Banking is disabled.
		return;
	} else if(mapdata[sd->bl.m].flag.nobanking) {
		clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1512),false,SELF); // Banking is disabled in this map.
		return;
	} else {
...
void clif_parse_BankClose(int fd, struct map_session_data *sd) {
	...
	if(!battle_config.feature_banking)
		clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1510),false,SELF); // Banking is disabled.
	if(mapdata[sd->bl.m].flag.nobanking)
		clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1512),false,SELF); // Banking is disabled in this map.
	if(sd->status.account_id == aid) {
		sd->state.banking = 0;
		clif_bank_close(sd);
...
void clif_party_message(struct party_data *p, int account_id, const char *mes, int len)
{
	...
	if( i < MAX_PARTY ) {
		...
		clif_send(buf, len + 9, &sd->bl, PARTY);
	}
}
...
void clif_mvp_exp(struct map_session_data *sd, uint32 exp)
{
	...
	if(battle_config.mvp_exp_reward_message) {
		...
		sprintf(e_msg, msg_txt(sd, 389), exp);
...
void clif_guild_basicinfo(struct map_session_data *sd) {
	...
	safestrncpy((char *)WFIFOP(fd,46),g->name,NAME_LENGTH);
#if PACKETVER < 20160622
	safestrncpy((char *)WFIFOP(fd,70),g->master,NAME_LENGTH);
#endif
	safestrncpy((char *)WFIFOP(fd,70 + offset),msg_txt(sd,300 + guild_checkcastles(g)),16); // "'N' castles"
...
void clif_guild_memberlist(struct map_session_data *sd)
{
	...
	for( i = 0, c = 0; i < g->max_member; i++ ) {
		...
		safestrncpy((char *)WFIFOP(fd,c * size + 84),m->name,NAME_LENGTH);
#else
...
void clif_guild_allianceadded(struct guild *g,int idx)
{
	...
	safestrncpy((char *)WBUFP(buf,10),g->alliance[idx].name,NAME_LENGTH);
 ...
 */
const char *clif_process_chat_message(struct map_session_data *sd, char *out_buf, int out_buflen)
{
	const char *packetmsg = NULL, *srcname = NULL, *srcmessage = NULL, *message = NULL;
	int packetlen = 0, textlen = 0, namelen = 0, messagelen = 0;
	int fd = sd->fd;
	struct s_packet_db *info = &packet_db[RFIFOW(fd,0)];
	...
	packetmsg = (char *)RFIFOP(fd,info->pos[1]);
	...
	textlen = packetlen - 4; //Packet doesn't include a NUL terminator
#else
	textlen = packetlen - 4 - 1;
#endif
	...
	srcmessage = packetmsg + namelen + 3; // <Name> " : " <Message>
	...
	safestrncpy(out_buf, packetmsg, textlen + 1); //[!] packetmsg is not necessarily NUL terminated
	message = out_buf + namelen + 3;

	if( !pc_process_chat_message(sd, message) )
		return NULL;
 ...
 *              shan't be used.
 */
bool clif_process_whisper_message(struct map_session_data *sd, char *out_name, char *out_message, int out_messagelen)
{
	const char *packetname = NULL, *packetmsg = NULL;
	int packetlen = 0, namelen = 0, messagelen = 0;
	int fd = sd->fd;
	struct s_packet_db *info = &packet_db[RFIFOW(fd,0)];
	...
	packetlen = RFIFOW(fd,info->pos[0]);
	packetname = (char *)RFIFOP(fd,info->pos[1]);
	packetmsg = (char *)RFIFOP(fd,info->pos[2]);

	if( packetlen < NAME_LENGTH + 4 + 1 ) {
...
#if PACKETVER >= 20151104
	messagelen = packetlen - NAME_LENGTH - 4;
#else
	messagelen = packetlen - NAME_LENGTH - 4 - 1;
#endif

	if( messagelen >= CHAT_SIZE_MAX || messagelen >= out_messagelen ) {
		ShowWarning("clif_process_whisper_message: Player '%s' sent a message too long ('%.*s')!\n", sd->status.name, CHAT_SIZE_MAX - 1, packetmsg);
		return false;
	}

	safestrncpy(out_name, packetname, namelen + 1); //[!] packetname is not NUL terminated
	safestrncpy(out_message, packetmsg, messagelen + 1);

	if( !pc_process_chat_message(sd, out_message) )
		return false;
...
{
	if( battle_config.pk_mode && battle_config.pk_mode_mes && sd && mapdata[sd->bl.m].flag.pvp ) {
		if( (int)sd->status.base_level < battle_config.pk_min_level ) {
			char output[CHAT_SIZE_MAX];

			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd, 218), // You've entered a PK Zone (safe until level %d).
				battle_config.pk_min_level);
			clif_showscript(&sd->bl, output, SELF);
		} else
			clif_showscript(&sd->bl, msg_txt(sd, 217), SELF); // You've entered a PK Zone.
	}
}
...
void clif_parse_WantToConnection(int fd, struct map_session_data *sd)
{
	...
	if( (bl = map_id2bl(account_id)) && bl->type != BL_PC ) {
...
void clif_parse_LoadEndAck(int fd,struct map_session_data *sd) {
	...
	if(mapdata[sd->bl.m].users++ == 0 && battle_config.dynamic_mobs)
		map_spawnmobs(sd->bl.m);

	if(pc_has_permission(sd,PC_PERM_VIEW_HPMETER)) {
		mapdata[sd->bl.m].hpmeter_visible++;
	...
	if(mapdata[sd->bl.m].flag.pvp && !pc_isinvisible(sd)) {
		...
		clif_map_property(&sd->bl,MAPPROPERTY_FREEPVPZONE,SELF);
	else if(mapdata[sd->bl.m].flag.gvg_dungeon)
	...
	if(sd->state.connect_new) {
		...
		if(night_flag && mapdata[sd->bl.m].flag.nightenabled) {
			sd->state.night = 1;
	...
	if(sd->state.changemap) { //Restore information that gets lost on map-change
		...
		if((battle_config.bg_flee_penalty != 100 || battle_config.gvg_flee_penalty != 100) &&
			(map_flag_gvg2(sd->state.pmap) || map_flag_gvg2(sd->bl.m) || mapdata[sd->state.pmap].flag.battleground || mapdata[sd->bl.m].flag.battleground))
		...
		if(mapdata[sd->bl.m].flag.battleground) {
			clif_map_type(sd,MAPTYPE_BATTLEFIELD); //Battleground Mode
			if(mapdata[sd->bl.m].flag.battleground == 2)
				clif_bg_updatescore_single(sd);
		}
		if(mapdata[sd->bl.m].flag.allowks && !map_flag_ks(sd->bl.m)) {
			char output[128];
		...
		if (!sd->state.connect_new &&
			...
			mapdata[sd->state.pmap].adjust.bexp != mapdata[sd->bl.m].adjust.bexp)
		{
		...
		if(channel_config.map_tmpl.name[0] && (channel_config.map_tmpl.opt&CHAN_OPT_AUTOJOIN) &&
			!mapdata[sd->bl.m].flag.nochmautojoin && !mapdata[sd->bl.m].instance_id)
	...
	if(!sd->state.autotrade && mapdata[sd->bl.m].flag.loadevent) //Lance
	...
	if((sd->sc.data[SC_MONSTER_TRANSFORM] || sd->sc.data[SC_ACTIVE_MONSTER_TRANSFORM]) &&
		battle_config.mon_trans_disable_in_gvg && map_flag_gvg2(sd->bl.m)) {
...
void clif_parse_QuitGame(int fd, struct map_session_data *sd)
{
	...
	if( !pc_ishiding(sd) && !sd->sc.data[SC_SUHIDE] &&
		(!battle_config.prevent_logout || !sd->canlog_tick || DIFF_TICK(gettick(), sd->canlog_tick) > battle_config.prevent_logout) ) {
		set_eof(fd);
...
void clif_parse_GlobalMessage(int fd, struct map_session_data *sd)
{
	...
	message = clif_process_chat_message(sd, full_message, sizeof full_message);
	if( !message )
		return;
	...
	//Send message back to the speaker
...
void clif_parse_WisMessage(int fd, struct map_session_data *sd)
{
	...
	if( !clif_process_whisper_message(sd, target, message, sizeof message) )
		return;
	...
	if( target[0] && (strncasecmp(target,"NPC:",4) == 0) && (strlen(target) > 4) ) {
		...
		if( (npc = npc_name2id(str)) ) {
			...
			for( i = 0; i < NUM_WHISPER_VAR; ++i ) { //Splits the message using '#' as separators
				split = strchr(str,'#');
				if( !split ) { //Use the remaining string
	...
	if( pc_get_group_level(sd) <= pc_get_group_level(dstsd) ) {
		...
		ARR_FIND(0, MAX_IGNORE_LIST, i, dstsd->ignore[i].name[0] == '\0' || !strcmp(dstsd->ignore[i].name, sd->status.name));
...
void clif_parse_ChangeChatOwner(int fd, struct map_session_data *sd)
{
	...
	chat_changechatowner(sd,(char *)RFIFOP(fd,packet_db[RFIFOW(fd,0)].pos[1]));
}
...
{
	chat_kickchat(sd,(char *)RFIFOP(fd,packet_db[RFIFOW(fd,0)].pos[0]));
}
...
void clif_parse_CreateParty(int fd, struct map_session_data *sd)
{
	...
	if( mapdata[sd->bl.m].flag.partylock ) { //Party locked
...
void clif_parse_CreateGuild(int fd,struct map_session_data *sd)
{
	...
	char *name = (char *)RFIFOP(fd,packet_db[RFIFOW(fd,0)].pos[1]);
	name[NAME_LENGTH - 1] = '\0';

	if( mapdata[sd->bl.m].flag.guildlock ) { //Guild locked
		clif_displaymessage(fd, msg_txt(sd, 228));
		return;
	}

	if( sd->clan )
		return; //Should display a clientside message "You are currently joined in Clan !!" so we ignore it
...
int clif_sub_guild_invite(int fd, struct map_session_data *sd, struct map_session_data *t_sd)
{
	...
	if( t_sd && t_sd->clan ) // Players in a clan can not join a guild
		return 1;
...
{
	struct map_session_data *t_sd = map_nick2sd((char *)RFIFOP(fd, packet_db[RFIFOW(fd,0)].pos[0]));
...
void clif_parse_GuildExpulsion(int fd,struct map_session_data *sd)
{
	...
	if( mapdata[sd->bl.m].flag.guildlock || sd->bg_id ) { // Guild locked
		clif_displaymessage(fd, msg_txt(sd, 228));
		return;
	}
...
void clif_parse_GuildBreak(int fd, struct map_session_data *sd)
{
	...
	guild_break(sd,(char *)RFIFOP(fd,packet_db[RFIFOW(fd,0)].pos[0]));
}
...
{ //FIXME: remove is supposed to receive account name for clients prior 20100803RE
	...
	player_name = (char *)RFIFOP(fd,packet_db[RFIFOW(fd,0)].pos[0]);
...
void clif_parse_FriendsListAdd(int fd, struct map_session_data *sd)
{
	...
	f_sd = map_nick2sd((char *)RFIFOP(fd,packet_db[RFIFOW(fd,0)].pos[0]));
...
{
	hom_change_name(sd,(char *)RFIFOP(fd,packet_db[RFIFOW(fd,0)].pos[0]));
}
...
void clif_parse_Check(int fd, struct map_session_data *sd)
{
	...
	if( !(pl_sd = map_nick2sd(charname)) || pc_get_group_level(sd) < pc_get_group_level(pl_sd) )
		return;
...
void clif_Mail_setattachment(struct map_session_data *sd, int index, int amount, uint8 flag)
{
	...
	WFIFOW(fd,46) = total;
	memset(WFIFOP(fd,48), 0, 5); //5 empty unknown bytes
	WFIFOSET(fd,packet_len(0xa05));
#endif
}
...
void clif_mail_getattachment(struct map_session_data *sd, struct mail_message *msg, uint8 result, enum mail_attachment_type type)
{
	...
	switch( type ) {
		case MAIL_ATT_ZENY:
			len = packet_len(0x9f2);
			cmd = 0x9f2;
			break;
		case MAIL_ATT_ITEM:
			len = packet_len(0x9f4);
			cmd = 0x9f4;
			break;
		default:
			return;
	...
	//clif_Mail_refreshinbox(sd, (enum mail_inbox_type)msg->type, 0);
#endif
}
...
#else
	WFIFOHEAD(fd,packet_len(0x9ed));
	WFIFOW(fd,0) = 0x9ed;
	WFIFOB(fd,2) = result;
	WFIFOSET(fd,packet_len(0x9ed));
#endif
}
...
void clif_mail_delete(struct map_session_data *sd, struct mail_message *msg, bool success)
{
	...
	WFIFOW(fd,0) = 0x257;
	WFIFOL(fd,2) = msg->id;
	WFIFOW(fd,6) = (success == false);
	WFIFOSET(fd,packet_len(0x257));
#else
	if( success ) { //This is only a success notification
		WFIFOHEAD(fd,packet_len(0x9f6));
		WFIFOW(fd,0) = 0x9f6;
		WFIFOB(fd,2) = msg->type;
		WFIFOQ(fd,3) = msg->id;
		WFIFOSET(fd,packet_len(0x9f6));
	}
#endif
}
...
void clif_Mail_refreshinbox(struct map_session_data *sd, enum mail_inbox_type type, uint64 mailId)
{
	...
	for( i = j = 0; i < MAIL_MAX_INBOX && j < md->amount; i++ ) {
		...
		safestrncpy((char *)WFIFOP(fd,53 + 73 * j), msg->send_name, NAME_LENGTH);
	...
	if( mailId ) {
		...
		//Unknown mail
		if( i == md->amount ) //Ignore the request for now
...
void clif_parse_Mail_refreshinbox(int fd, struct map_session_data *sd)
{
	...
	switch( openType ) {
		...
		case MAIL_INBOX_RETURNED:
			break;
		default:
			return; //Unknown type: ignore request
	}
	...
	if( cmd != 0x9ee ) //If it is not a next page request
		mailId = 0;
...
/// Mail inbox close request (CZ_CLOSE_MAILBOX).
/// 09e9
void clif_parse_Mail_close(int fd, struct map_session_data *sd) {
	clif_Mail_new(sd, 0, NULL, NULL);
}
...
{ //@TODO: Packet description => for repeated block
	...
	if( i == MAIL_MAX_INBOX ) {
		...
		if( item->nameid && (data = itemdb_exists(item->nameid)) ) {
		...
		safestrncpy((char *)WFIFOP(fd,24), msg->body, msg_len);

		offset = 24 + msg_len;

		for( j = 0; j < MAIL_MAX_ITEM; j++ ) {
			item = &msg->item[j];
			if( item->nameid > 0 && item->amount > 0 && (data = itemdb_exists(item->nameid)) ) {
...
void clif_send_Mail_beginwrite_ack(struct map_session_data *sd, char *name, bool success)
{
	...
	WFIFOHEAD(fd,packet_len(0xa12));
	WFIFOW(fd,0) = 0xa12;
	safestrncpy((char *)WFIFOP(fd,2), name, NAME_LENGTH);
	WFIFOB(fd,26) = success;
	WFIFOSET(fd,packet_len(0xa12));
}
...
void clif_parse_Mail_beginwrite(int fd, struct map_session_data *sd)
{
	...
	if( sd->npc_id || sd->state.storage_flag || sd->state.mail_writing || sd->trade_partner ) {
...
void clif_Mail_Receiver_Ack(struct map_session_data *sd, uint32 char_id, short class_, uint32 level, const char *name)
{
	...
	WFIFOW(fd,8) = level;
#if PACKETVER >= 20160302
	strncpy((char *)WFIFOP(fd,10), name, NAME_LENGTH);
#endif
	WFIFOSET(fd,packet_len(cmd));
}
...
void clif_parse_Mail_getattach(int fd, struct map_session_data *sd)
{
	...
	if( attachment&MAIL_ATT_ITEM ) {
		...
		if( i == MAIL_MAX_ITEM ) //No items were found
			attachment &= ~MAIL_ATT_ITEM;
	}

	if( attachment == MAIL_ATT_NONE )
		return; //Either no attachment requested at all or there are no zeny or items in the mail
	...
	if( attachment&MAIL_ATT_ZENY ) {
		if( msg->zeny + sd->status.zeny > MAX_ZENY ) {
			...
			return;
		} else //To make sure another request fails
			msg->zeny = 0;
	}
	...
	if( attachment&MAIL_ATT_ITEM ) {
		...
		for( i = 0; i < MAIL_MAX_ITEM; i++ ) {
			...
			if( item->nameid > 0 && item->amount > 0 ) {
				...
				if( !(data = itemdb_exists(item->nameid)) )
					continue;
		...
		memset(msg->item, 0, MAIL_MAX_ITEM * sizeof(struct item)); //To make sure another request fails
	}
...
void clif_parse_Mail_return(int fd, struct map_session_data *sd)
{
	...
	ARR_FIND(0, MAIL_MAX_INBOX, i, sd->mail.inbox.msg[i].id == mail_id);
	if( i < MAIL_MAX_INBOX && sd->mail.inbox.msg[i].send_id )
...
void clif_parse_Mail_send(int fd, struct map_session_data *sd)
{
	...
	if( length < 0x3e ) {
		ShowWarning("clif_parse_Mail_send: Packet length too short...\n");
		clif_Mail_send(sd, WRITE_MAIL_FAILED);
		return;
	}

	if( !sd->state.mail_writing ) //Forged request without a begin writing packet?
		return; //Ignore it
...
void clif_parse_Auction_setitem(int fd, struct map_session_data *sd)
{
	...
	if( (item = itemdb_exists(sd->inventory.u.items_inventory[idx].nameid)) &&
		!(item->type == IT_ARMOR || item->type == IT_PETARMOR || item->type == IT_WEAPON ||
		item->type == IT_CARD || item->type == IT_ETC || item->type == IT_SHADOWGEAR) )
	{
		clif_Auction_setitem(sd->fd, idx, true);
		return; //Consumable or pets are not allowed
	}
...
void clif_cashshop_open(struct map_session_data *sd) {
	if( mapdata[sd->bl.m].flag.nocashshop ) {
		clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1511),false,SELF); // Cash Shop is disabled in this map.
		return;
	}
	WFIFOHEAD(sd->fd, 10);
			...
			for( j = 0; j < qi->objective_count; j++ ) {
				...
				if( qi->objectives[j].min_level ) {
					if( mob_type ) // %s Lv.%d~Lv.%d
						safesnprintf(target, sizeof(target), msg_txt(sd, 236), clif_mobtype_name((enum e_mob_type)mob_type), qi->objectives[j].min_level, qi->objectives[j].max_level);
					else // Lv.%d~Lv.%d
						safesnprintf(target, sizeof(target), msg_txt(sd, 234), qi->objectives[j].min_level, qi->objectives[j].max_level);
				} else
...
void clif_quest_add(struct map_session_data *sd, struct quest *qd)
{
	...
	for( i = 0, offset = 17; i < qi->objective_count; i++ ) {
		...
		if( qi->objectives[i].min_level ) {
			if( mob_type ) // %s Lv.%d~Lv.%d
				safesnprintf(target, sizeof(target), msg_txt(sd, 236), clif_mobtype_name((enum e_mob_type)mob_type), qi->objectives[i].min_level, qi->objectives[i].max_level);
			else // Lv.%d~Lv.%d
				safesnprintf(target, sizeof(target), msg_txt(sd, 234), qi->objectives[i].min_level, qi->objectives[i].max_level);
		} else
			safesnprintf(target, sizeof(target), clif_mobtype_name((enum e_mob_type)mob_type));
...
void clif_bg_updatescore(int16 m)
{
	...
	WBUFW(buf,0) = 0x2de;
	WBUFW(buf,2) = mapdata[m].bgscore_lion;
	WBUFW(buf,4) = mapdata[m].bgscore_eagle;
...
void clif_bg_updatescore_single(struct map_session_data *sd)
{
	...
	WFIFOW(fd,0) = 0x2de;
	WFIFOW(fd,2) = mapdata[sd->bl.m].bgscore_lion;
	WFIFOW(fd,4) = mapdata[sd->bl.m].bgscore_eagle;
...
void clif_sub_ranklist(unsigned char *buf, int idx, struct map_session_data *sd, enum e_rank rankingtype) {
	...
	switch(rankingtype) {
		...
		case RANK_KILLER: list = NULL; size = 0; break; //PK currently unsupported
		default:
			ShowError("clif_sub_ranklist: Unsupported ranking type '%d'. Please report this.\n", rankingtype);
			return;
	}

	for(i = 0; i < min(10, size); i++) { //Packet size limits this list to 10 elements [Skotlex]
		if(list[i].id > 0) {
			const char *name;

			if(!strcmp(list[i].name, "-") && (name = map_charid2nick(list[i].id)))
				safestrncpy((char *)WBUFP(buf,idx + NAME_LENGTH * i), name, NAME_LENGTH);
			else
				safestrncpy((char *)WBUFP(buf,idx + NAME_LENGTH  * i), list[i].name, NAME_LENGTH);
		} else
			safestrncpy((char *)WBUFP(buf,idx + NAME_LENGTH * i), "None", NAME_LENGTH);
		WBUFL(buf,idx + NAME_LENGTH * 10 + i * 4) = list[i].fame; //Points
	}

	for(; i < 10; i++) { //In case the MAX is less than 10
		safestrncpy((char *)WBUFP(buf,idx + NAME_LENGTH * i), "Unavailable", NAME_LENGTH);
...
void clif_ranklist(struct map_session_data *sd, int16 rankingType) {
	...
	rank = (enum e_rank)rankingType; //Can be safely casted here, since we validated it before
	WBUFW(buf,0) = cmd;
...
void clif_display_pinfo(struct map_session_data *sd, int cmdtype) {
	if(sd) {
		 ...
		 */
		//0:PCRoom
		details_bexp[0] = mapdata[sd->bl.m].adjust.bexp;
...
void clif_clan_message(struct clan *clan, const char *mes, int len) {
	...
	if( (sd = clan_getavailablesd(clan)) )
...
void clif_clan_onlinecount(struct clan *clan) {
	...
	if( (sd = clan_getavailablesd(clan)) )
		clif_send(buf, packet_len(0x988), &sd->bl, CLAN);
#endif
}
...
void clif_merge_item_ack(struct map_session_data *sd, unsigned short index, unsigned short count, enum MERGE_ITEM_ACK type) {
	...
	if( !(info = &packet_db[cmd]) || !info->len )
		return;
...
void clif_parse_merge_item_req(int fd, struct map_session_data *sd) {
	...
	if( !(info = &packet_db[RFIFOW(fd,0)]) || !info->len )
		return;
...
void clif_parse_roulette_open(int fd, struct map_session_data *sd) {
	...
	if( !battle_config.feature_roulette ) {
		clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,717),false,SELF); // Roulette is disabled
		return;
	}
...
void clif_sale_start(struct sale_item_data *sale_item, struct block_list *bl, enum send_target target) {
	...
	clif_send(buf, packet_len(0x9b2), bl, target);
#endif
}
...
void clif_sale_end(struct sale_item_data *sale_item, struct block_list *bl, enum send_target target) {
	...
	clif_send(buf, packet_len(0x9b3), bl, target);
#endif
}
...
void clif_sale_amount(struct sale_item_data *sale_item, struct block_list *bl, enum send_target target) {
	...
	clif_send(buf, packet_len(0x9c4), bl, target);
#endif
}
...
void clif_parse_sale_search(int fd, struct map_session_data *sd) {
	...
	clif_sale_search_reply(sd, NULL); // Not found
#endif
}
	...
	switch( RFIFOB(fd,2) ) {
		...
		case IN_UI_ATTENDANCE:
			if( !pc_has_permission(sd, PC_PERM_ATTENDANCE) )
				clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,767),false,SELF); // You are not allowed to use the attendance system.
...
void clif_parse_private_airship_request(int fd, struct map_session_data *sd) {
	...
	if( !battle_config.feature_privateairship ) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 768), false, SELF); // The private airship system is disabled.
		return;
	}
 ...
 */
static inline bool clif_refineui_materials_sub(struct item *item, struct item_data *id, struct s_refine_materials materials[REFINEUI_MAT_MAX], int index, enum e_refine_cost_type type) {
	//In official kRO, HD materials have same success chance like Enriched materials for refining items till +10
	bool is_enriched = false;
	struct s_refine_cost *cost = NULL;

	if( index < 0 || index >= REFINEUI_MAT_MAX )
		return false;

	if( !(cost = status_refine_cost((enum e_refine_type)id->wlv, type)) )
		return false;

	memcpy(&materials[index].cost, cost, sizeof(struct s_refine_cost));

	switch( type ) {
		case REFINE_COST_TYPE_HD:
		case REFINE_COST_TYPE_ENRICHED:
		case REFINE_COST_TYPE_LIMITED_HD:
			is_enriched = true;
			break;
	}
	materials[index].chance = status_get_refine_chance((enum e_refine_type)id->wlv, item->refine, is_enriched);

	//Either of the values was not set
	if( !materials[index].cost.nameid || !materials[index].cost.zeny || !materials[index].chance ) {
		//Reset all entries properly
		materials[index].cost.nameid = materials[index].cost.zeny = materials[index].chance = 0;
		return false;
	} else //Everything was set properly
		return true;
}
 ...
 */
static inline uint8 clif_refineui_materials(struct item *item, struct item_data *id, struct s_refine_materials materials[REFINEUI_MAT_MAX]) {
	//Initialize the counter
	uint8 count = 0;

	//Zero the memory
	memset(materials, 0, sizeof(struct s_refine_materials) * REFINEUI_MAT_MAX);

	if( item->refine >= 10 ) {
		//Normal refine requirements for +10 and above
		if( clif_refineui_materials_sub(item, id, materials, count, REFINE_COST_TYPE_OVER10) )
			count++;
		//HD refine requirements for +10 and above
		if( clif_refineui_materials_sub(item, id, materials, count, REFINE_COST_TYPE_OVER10_HD) )
			count++;
	} else {
		//Normal refine requirements
		if( clif_refineui_materials_sub(item, id, materials, count, REFINE_COST_TYPE_NORMAL) )
			count++;
		//HD refine requirements
		if( item->refine >= 7 ) {
			if( clif_refineui_materials_sub(item, id, materials, count, REFINE_COST_TYPE_HD) )
				count++;
			if( clif_refineui_materials_sub(item, id, materials, count, REFINE_COST_TYPE_LIMITED_HD) )
				count++;
		}
		//Enriched refine requirements
		if( clif_refineui_materials_sub(item, id, materials, count, REFINE_COST_TYPE_ENRICHED) )
			count++;
	}

	//Blacksmith Blessing requirements if any
	status_get_refine_blacksmithBlessing(&materials[REFINEUI_MAT_BS_BLESSING].bs_blessing, (enum e_refine_type)id->wlv, item->refine);
...
void clif_refineui_info(struct map_session_data *sd, uint16 index) {
	...
	if( itemtype(id->nameid) != IT_WEAPON && itemtype(id->nameid) != IT_ARMOR )
		return;
	...
	WFIFOW(fd,2) = length;
	WFIFOW(fd,4) = index + 2;
	WFIFOB(fd,6) = (uint8)materials[REFINEUI_MAT_BS_BLESSING].bs_blessing.count;
...
void clif_parse_refineui_refine(int fd, struct map_session_data *sd) {
	...
	if( rnd()%100 < materials[i].chance ) {
		...
		do {
			if( !use_bs_blessing ) {
				if( materials[i].cost.downrefine_chance < rnd()%100 ) {
...
void clif_parse_equipswitch_request(int fd, struct map_session_data *sd)
{
	...
	if( i == EQI_MAX ) {
		clif_equipswitch_reply(sd, false); //Client will show: "There is no item to replace." and should not even come here
		return;
	}
...
void DumpUnknow(int fd, TBL_PC *sd, int cmd, int packet_len) {
	...
	FILE *fp;
	const time_t time_server = time(NULL); //Get time in seconds since 1/1/1970
	struct tm datetime;
	char datestr[512];
...
static int clif_parse(int fd)
{
	...
	for( pnum = 0; pnum < 3; ++pnum ) { //Begin main client packet processing loop
...
#ifdef PACKET_OBFUSCATION
		if( sd ) //Check if it is a player that tries to connect to the map server
		...
		if( cmd > MAX_PACKET_DB || cmd < MIN_PACKET_DB || packet_db[cmd].len == 0 ) {
			...
			if( !sd ) { //If it is a connection attempt, check if the packet would have been valid without decrypting it
#ifdef PACKET_OBFUSCATION
		...
		if( packet_db[cmd].func == clif_parse_debug )
			...
			else if( sd && !sd->bl.prev && packet_db[cmd].func != clif_parse_LoadEndAck )
...
void do_init_clif(void) {
	const char *colors[COLOR_MAX] = { "0x00FF00", "0xFF0000", "0xFFFFFF", "0xFFFF00", "0x00FFFF", "0xB5FFB5" };
	int i;
