...
struct achievement *mapif_achievements_fromsql(uint32 char_id, int *count)
{
	...
	StringBuf_Printf(&buf, " FROM `%s` WHERE `char_id` = '%u'", achievement_db, char_id);
	...
	*count = (int)SqlStmt_NumRows(stmt);
	if( *count > 0 ) {
		...
		if( i < *count ) { // Should never happen, compact array
			*count = i;
...
{
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` = '%d' AND `char_id` = '%u'", achievement_db, achievement_id, char_id) ) {
...
bool mapif_achievement_add(uint32 char_id, struct achievement *ad)
{
	...
	StringBuf_Init(&buf);
	StringBuf_Printf(&buf, "INSERT INTO `%s` (`char_id`, `id`, `completed`, `rewarded`", achievement_db);
	for( i = 0; i < MAX_ACHIEVEMENT_OBJECTIVES; ++i )
		StringBuf_Printf(&buf, ", `count%d`", i + 1);
...
int mapif_parse_achievement_reward(int fd)
{
	...
	if( Sql_Query(sql_handle, "UPDATE `%s` SET `rewarded` = FROM_UNIXTIME('%u') WHERE `char_id`='%u' AND `id` = '%d' AND `completed` IS NOT NULL AND `rewarded` IS NULL", achievement_db, (uint32)current, char_id, achievement_id) == SQL_ERROR ||
