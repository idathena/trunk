...
static TIMER_FUNC(guild_save_timer)
{
...
struct guild *inter_guild_fromsql(int guild_id)
{
	...
	for( i = 0; i < MAX_GUILDSKILL; i++ ) 
		g->skill[i].id = i + GD_SKILLBASE; //Skill IDs must always be initialized [Skotlex]
...
int inter_guild_sql_init(void)
{
	...
	//Read exp file
	sv_readdb("db", DBPATH"exp_guild.txt", ',', 1, 1, MAX_GUILDLEVEL, exp_guild_parse_row);
...
int mapif_parse_GuildBasicInfoChange(int fd, int guild_id, int type, const char *data, int len)
{
	...
	if( !(g = inter_guild_fromsql(guild_id)) )
		return 0;
