...
{
	return memitemdata_to_sql(p->u.items_inventory, MAX_INVENTORY, char_id, TABLE_INVENTORY, p->stor_id);
}
...
{
	return memitemdata_to_sql(p->u.items_cart, MAX_CART, char_id, TABLE_CART, p->stor_id);
}
...
{
	return memitemdata_to_sql(p->u.items_storage, MAX_STORAGE, account_id, TABLE_STORAGE, p->stor_id);
}
 ...
 * @return 0 if success, or error count
 */
static int guild_storage_tosql(int guild_id, struct s_storage *p)
{
	//ShowInfo("Guild Storage has been saved (GID: %d)\n", guild_id);
	return memitemdata_to_sql(p->u.items_guild, inter_guild_storagemax(guild_id), guild_id, TABLE_GUILD_STORAGE, p->stor_id);
}
...
{
	return memitemdata_from_sql(p, MAX_INVENTORY, char_id, TABLE_INVENTORY, p->stor_id);
}
...
{
	return memitemdata_from_sql(p, MAX_CART, char_id, TABLE_CART, p->stor_id);
}
...
{
	return memitemdata_from_sql(p, MAX_STORAGE, account_id, TABLE_STORAGE, p->stor_id);
}
 ...
 * @return True if success, False if failed
 */
static bool guild_storage_fromsql(int guild_id, struct s_storage *p)
{
	return memitemdata_from_sql(p, inter_guild_storagemax(guild_id), guild_id, TABLE_GUILD_STORAGE, p->stor_id);
}
...
static void inter_storage_checkDB(void)
{
	...
	for( i = 0; i < interserv_config.storage_count; i++ ) {
		if( !&interserv_config.storages[i] || !interserv_config.storages[i].name ||
			!interserv_config.storages[i].table || *interserv_config.storages[i].table == '\0' )
			continue;
