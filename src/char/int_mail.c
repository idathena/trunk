...
int mail_savemessage(struct mail_message *msg)
{
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf,"INSERT INTO `%s` (`id`, `index`, `amount`, `nameid`, `refine`, `attribute`, `identify`, `unique_id`, `bound`", mail_attachment_db);
...
static int mail_timer_sub(int limit, enum mail_inbox_type type)
{
	...
	for( i = 0; i < MAIL_MAX_INBOX; i++ ) {
		...
		if( (character = (struct online_char_data *)idb_get(online_char_db, mails[i].account_id)) && character->server >= 0 )
			map_fd = server[character->server].fd;
		else
			map_fd = 0;
 ...
 *------------------------------------------*/
static void mapif_Mail_sendinbox(int fd, int char_id, unsigned char flag, enum mail_inbox_type type)
{
	struct mail_data md;
...
static void mapif_Mail_return(int fd, uint32 char_id, int mail_id)
{
	...
	if( mail_loadmessage(mail_id, &msg) ) {
		...
		||  SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` = '%d'", mail_attachment_db, mail_id) )
			Sql_ShowDebug(sql_handle);
		else if( msg.send_id > 0 ) { // If it was not sent by the server, since we do not want to return mails to the server
...
static void mapif_parse_Mail_send(int fd)
{
	...
	if( msg.dest_id > 0 ) {
		if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`, `name` FROM `%s` WHERE `char_id` = '%u'", char_db, msg.dest_id) ) {
			Sql_ShowDebug(sql_handle);
			return;
		}
		msg.dest_id = 0;
